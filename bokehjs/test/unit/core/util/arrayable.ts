...
describe("core/util/arrayable module", () => {
  ...
  it("should support minmax() function", () => {
    expect(arrayable.minmax([])).to.be.deep.equal([Infinity, -Infinity])
    expect(arrayable.minmax([NaN])).to.be.deep.equal([Infinity, -Infinity])
    expect(arrayable.minmax([1, 2, 3])).to.be.deep.equal([1, 3])
    expect(arrayable.minmax([3, 2, 1])).to.be.deep.equal([1, 3])
    expect(arrayable.minmax([1, 2, NaN, 3])).to.be.deep.equal([1, 3])
    expect(arrayable.minmax([3, 2, NaN, 1])).to.be.deep.equal([1, 3])
  })
})
